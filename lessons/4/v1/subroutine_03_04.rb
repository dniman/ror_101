...
class Subroutine0304 < Routine
  def run
    ...
    if stations.empty?
      ...
      text = []
      stations.each_with_index { |station, i| text << "  #{i + 1}.#{station.name}" }
  ...
  def routes_menu
    ...
    if App.routes.empty?
      ...
      App.routes.each_with_index { |route, i| text << "  #{i + 1}.#{route.stations.map(&:name).join('-')}" }
