...
class MainMenu < TextMenu
  ...
  def train_action
    @train_menu = TrainMenu.new.tap do |m|
      ...
      m.sections << '  8. Просмотреть список вагонов поезда'
      m.sections << '  9. Забронировать место/зарезервировать объем в поезде'
      m.sections << '  0. Вернуться в предыдущее меню'
      m.footer = '>> '
      ...
      m.actions << :show_train_carriages
      m.actions << :reserve_space
    end
